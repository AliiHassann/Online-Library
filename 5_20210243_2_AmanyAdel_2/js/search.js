...
function SearchB(test) {
  ...
  for (let i = 0; i < books.length; i++) {
    if (books[i].name.toLowerCase().includes(test.toLowerCase()) == true || books[i].author.toLowerCase().includes(test.toLowerCase())==true || books[i].category.toLowerCase().includes(test.toLowerCase())==true) {
      temp += `<tr>
            <td>${i + 1}</td>
            <td>${books[i].id}</td>
